...
- [ ] In one out the other (int& + const int& aliasing same var) — 02/09 — MISSED per platform record (session log had it as ok; platform kept 2026-09-07). Prints 1: both references alias the same int, so the write through the non-const one is visible through the const one. Anki: "int x=0; int& a=x; const int& b=x; a=1; b?" / "1. const on a reference restricts that path only; the object changed."
- [ ] East to West (`const int_ptr foo` vs `const int* bar`) — 07/09 — MISSED per platform record (REPEAT of the 29/08 quiz miss). Anki: "`using int_ptr = int*; const int_ptr p;` — type of p?" / "`int* const`: const on an alias qualifies the whole aliased type, so it lands on the pointer." `foo` is `int* const` (const on an alias is top-level on the whole aliased type), `bar` is `const int*`. Same fact as the 29/08 quiz miss "const alias = top-level const".
