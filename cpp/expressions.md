...
When the two branches are prvalues of different class types, the compiler tries to build an implicit conversion sequence from each branch to the other branch's type. If exactly one direction is possible, the conditional has that type and the conversion is performed on the chosen branch; if both directions are possible, or neither, the expression is ill-formed. So with `struct C {}; struct D { D(const C&); };`, the expression `flag ? C{} : D{}` has type `D`, and a function declared `auto f(bool flag) { return flag ? C{} : D{}; }` returns `D`. When `flag` is true, a `C` temporary is constructed and then a `D` is constructed from it through the converting constructor, so the output shows both constructors in that order, and `f(true).foo()` calls `D::foo`. Only one user-defined conversion may appear in the sequence, the same limit as for function arguments.
