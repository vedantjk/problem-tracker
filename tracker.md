...
| Graphs | Island Perimeter | Easy | Solved | 21/09/2026 | 15 | 4 | NC250. W5 graph week. Solved with a DFS plus visited grid, where each land cell adds 4 minus its number of land neighbours. That works but the traversal is unnecessary: the per-cell formula does not depend on visit order or connectivity, so a plain double loop over the grid gives the same sum in O(m*n) with no recursion and no visited array. Equivalent closed form: 4 * land cells minus 2 * adjacent land pairs, counting only right and down neighbours so each pair is seen once. |
| Graphs | Verifying An Alien Dictionary | Easy | Solved | 21/09/2026 | 20 | 3 | NC250. W5 graph week (warm-up for Alien Dictionary). Build rank[26] from the order string, then compare each adjacent pair: walk to the first differing index j, and if one word ran out first the shorter must come first (prefix rule), otherwise compare rank[a[j]] against rank[b[j]]. Sorted means every adjacent pair is in order, so pairwise checks are sufficient; O(total characters). Conf 3 because 20 min on an Easy, twice the budget; the time went on the prefix case ("apple" before "app"): the ordering is skip the common prefix first, then check whether either word ran out before comparing ranks. That same case is the invalid-order edge case in the Hard version, so it is now known. Cheaper alternative: a comparator that maps chars through rank and std::is_sorted(words, cmp), which is one line once the mapping exists. |
| Graphs | Find the Town Judge | Easy | Solved | 21/09/2026 | 10 | 4 | NC250. W5 graph week. Degree counting on a directed graph: the judge is the one node with out-degree 0 and in-degree n-1. Solved with two adjacency lists, which is correct but stores every edge when only the counts are needed. The tight version keeps one int per node, score[b]++ and score[a]-- per trust pair, and returns the node whose score is n-1: out-degree 0 and in-degree n-1 is the only way to reach that value. O(n + E) time, O(n) space, no allocations per edge. Watch the size() == n-1 comparison: size_t against int is a signed/unsigned mismatch that -Wall flags. |
| Graphs | Max Area of Island | Med | Solved | 21/09/2026 | 15 | 4 | NC250. W5 graph week. DFS flood fill that returns the size of the component: mark the cell by writing 0 into the grid before recursing (this is the visited mark, so no extra array), then return 1 plus the sum of the recursive calls on in-bounds land neighbours; the answer is the max over all start cells. O(m*n) time because each cell is sunk once. Mutating the input is fine on LeetCode but say it out loud in an interview, and offer a visited array or restoring the grid if the caller needs it intact. Recursion depth can reach m*n on a snake-shaped island, so an explicit stack or BFS is the safe version for large grids. |
...
| Graphs | Walls And Gates | Med | Solved | 21/09/2026 | 15 | 4 | NC250. W5 graph week. Multi-source BFS: seed the queue with every gate (0) at once, then expand level by level so the first time an INF room is reached its distance is the level number, which is the shortest distance to the nearest gate. Pushing only INF neighbours doubles as the visited check, since walls (-1) and already-reached rooms are never INF. O(m*n). Two tidy-ups: assigning rooms[i][j] = steps is enough, the min() is redundant because a cell is queued once; and pushing then marking can re-queue a cell twice in the same level if two neighbours reach it, so write the distance at push time (rooms[ii][jj] = steps + 1) rather than at pop time. Same template as Rotting Oranges and 01 Matrix. |
